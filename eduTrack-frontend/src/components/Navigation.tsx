...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  const menuItems: Array<{ id: TabId; label: string; icon: any; color: string; badge?: boolean }> = [
    { id: 'overview', label: 'Vue Globale', icon: GraduationCap, color: '#AFE3F4' }, // Light Cyan
    { id: 'imports', label: 'Gestion des Imports', icon: UploadCloud, color: '#B9DDF5' }, // Soft Ice Blue
    { id: 'analytics', label: 'Analyses & Clustering', icon: BarChart3, color: '#83C5F1' }, // Sky Blue
    { id: 'students', label: 'Directoire Étudiants', icon: Users, color: '#F4DBE3' }, // Soft Pink
    { id: 'alerts', label: 'Centre d\'Alertes', icon: BellRing, color: '#5EA8DA', badge: true }, // Medium Blue
    { id: 'reports', label: 'Rapports Professionnels', icon: FileText, color: '#AFE3F4' }, // Accent Light Cyan
  ];

  return (
    <aside 
      id="main-sidebar animate-fade-in"
      className="w-72 bg-slate-900 border-r border-slate-800 text-slate-100 flex flex-col justify-between"
    >
      {/* Top Brand Logo */}
      ...
      {/* Navigation Items */}
      <nav id="sidebar-nav" className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
          ...
          return (
            <button
              ...
              style={{
                ...
                color: isActive ? '#0F172A' : undefined // High-contrast WCAG AA Slate text
              }}
              title={item.label}
